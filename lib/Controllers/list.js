...
List.prototype.fetch = function(req, res, context) {
 ...
 // Checking attributes passed through params - if they're reference string or text field - search with like and not exact match
  for(let attr in extraSearchCriteria) {
      var attrType = model.rawAttributes[attr].type;
      if ((attrType instanceof Sequelize.STRING) ||
          (attrType instanceof Sequelize.TEXT)) {      
        var query = {};
        ...
        let searchOperator = Sequelize.Op.iLike 
        searchString = '%' + extraSearchCriteria[attr] + '%';

        query[searchOperator] = searchString;
        extraSearchCriteria[attr] = query;
      }
  }
